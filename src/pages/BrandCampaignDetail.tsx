...
const BrandCampaignDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for creator response updates (for real-time status changes)
    ...
    socket.on('content_approval_updated', (data) => {
      if (data.campaign_id === id) {
        fetchCampaignDetails(); // Refresh to get updated content
      }
    });
